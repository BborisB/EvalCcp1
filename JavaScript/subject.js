...
else
{
    ...
    {
        let currentSubject = currentCategory.subjects[localStorage.getItem("currentSubjectId")-1];
        if(currentSubject==null)
        {
            location.href = location.href.replace("subject.html", "category.html");
        }
        else
        {
            ...
            title.textContent = currentSubject.title;

            for(let i=0;i<currentSubject.messages.length;i++)
            {
            ...
            textAreaMessage.addEventListener("keydown", (e)=>
            {
                if(e.key=="Enter" && !e.shiftKey)
                {
                    ...
                    if(text)
                    {
                        ...
                        addMessage(message);
                        currentSubject.messages.push(message);
                        currentSubject.lastComment = message.date;
